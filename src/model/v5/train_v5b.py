"""
SC-CLM V5B Training Script
--------------------------
V5 architecture with ReactionT5 backbone, augment_n=5.
"""
...
_LOCAL_RT5 = _REPO_ROOT / "models" / "ReactionT5v2-forward"
# 仓库内有快照则用本地；否则用 HF（Linux 服务器常见）；也可用环境变量 V5B_REACTION_MODEL_PATH 覆盖。
BASE_MODEL = (
    str(_LOCAL_RT5)
    if _LOCAL_RT5.is_dir()
    else "sagawa/ReactionT5v2-forward"
)
OUTPUT_DIR = "results/checkpoints/v5b"
LOG_FILE = "logs/v5b/train.log"
AUGMENT_N = 5
...
logger = logging.getLogger("v5b_train")
...
def resolve_reaction_base_model() -> tuple[str, bool]:
    """Resolve ReactionT5 source: env override, repo-local snapshot, then HF id."""
    ...
    if local_snapshot:
        ...
        logger.warning(
            "Reaction model path is set but not found: %s", snapshot_path
        )

    candidate = Path(BASE_MODEL)
    if candidate.exists() and candidate.is_dir():
        logger.info("Using local ReactionT5 snapshot: %s", candidate)
        return str(candidate), True

    logger.info("Using remote ReactionT5 model id: %s", BASE_MODEL)
...
def main(config: V5Config = None):
    ...
    # ReactionT5v2-forward backbone uses d_model=768; conditioning dim must match.
    config.reaction_embed_dim = 768
    ...
    # Smoke test: ensure ReactionT5 encoder accepts current input format.
    with torch.no_grad():
        smoke = tokenizer("CCO", return_tensors="pt")
        _ = model.base_model.encoder(input_ids=smoke["input_ids"])
    logger.info("ReactionT5 encoder smoke test passed for input format.")
    ...
    logger.info(
        "Starting V5B training (%d epochs, effective batch=%d, CFG dropout=%.2f)...",
        config.num_epochs,
    ...
    logger.info("V5B training complete.")
